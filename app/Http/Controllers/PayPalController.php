    ...
    public function getExpressCheckoutSuccess(Request $request)
    {
        ...
        if (in_array(strtoupper($response['ACK']), ['SUCCESS', 'SUCCESSWITHWARNING'])) {
            ...
            if ($invoice->paid) {
                ...
                return view('payment/paypalSuccess');

            } else {

                session()->put(['code' => 'danger', 'message' => "Error processing PayPal payment for Order $invoice->id!"]);
                return view('payment/paypalError');
            }
...
*/
    ...
    protected function getCheckoutData($recurring = false)
    {
        ...
        if ($recurring === true) {
            ...
            $total = 0;
            $items['plans']=array('title' =>$getplans['title'],'qty'=>1);
            $items['packages']=array('title' =>$getpackage['title'],'price'=>$getpackage['price'],'qty'=>1);
            $total += $getpackage[$i]['price'] * 1;
            for($i=0;$i<count($getadditionalItems);$i++){
            $items['additional'][$i]=array('title' =>$getadditionalItems[$i]['title'],'price'=>$getadditionalItems[$i]['price'],'qty'=>1);
            ...
            $data['total'] = $total;
            $data['return_url'] = url('/paypal/ec-checkout-success');
            $data['invoice_description'] = json_encode($items);
            $data['cancel_url'] = url('/');
        }
