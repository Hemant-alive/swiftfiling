            ...
            <div class="row">
                <form method="POST" action="{{url('admin/plans/index')}}" id='' class="form-horizontal">
                    ...
                    <div class="col-sm-2 grid_box1"> 
                        ...
                        <input type="text" name="name" value="{{Session::has('search_title')? Session::get('search_title') : old('title')}}" class="form-control input-sm" placeholder="Plan Name">
                    </div>
                    <div class="col-sm-2 grid_box1">
        ...
        <div class="xs tabls tab-content">
            <div class="panel panel-warning">
                ...
                <div class="panel-body no-padding" style="display: block;">
                        ...
                        <tbody> 
                            ...
                            <?php if(isset($page)) $i=$page['from']; else $i=1; ?> @foreach($plans as $plan)
                                <tr>
                                    ...
                                    <td><a onclick="return confirm('Do you Want to Change Status?');return false;" href="{{URL :: asset('admin/plans/status/'.$plan->id.'_'.$plan->status)}}">{!! ($plan->status=='1')?'<i class="fa fa-toggle-on" aria-hidden="true"></i><span class="success">active</span>' : '<i class="fa fa-toggle-on" aria-hidden="true"></i><span class="text-danger">Inactive</span>'!!}</a></td>

                                    <td>{{date('F d, Y', strtotime($plan->created_at))}}</td>
                                    <td> <a href="{{URL :: asset('admin/plans/'.$plan->id.'/edit')}}" class="btn btn-success" title='edit'><i class="fa fa-edit"></i></a>
                   ...
                   <div class="text-center">{{ $plans->links() }}</div>
                </div>
            </div>
        </div>
    </div>
